...
mod tests {
    ...
    async fn report_failure_and_status_handlers_use_global_engine() {
        ...
        // report-failure: a model rewrite still queues for human approval even
        // when auto-decide is enabled.
        ...
        let v = body_json(resp).await;
        assert_eq!(v.get("action").and_then(Value::as_str), Some("queue_fix"));
        assert_eq!(v.get("corrected").and_then(Value::as_str), Some("cp"));
