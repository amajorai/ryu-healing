    ...
    async fn diagnose_verdict(
        ...
        match source {
            HealSource::Agent { agent_id } => {
                ...
                // A side model may suggest a materially different instruction,
                // but that rewrite is not an unattended retry. Auto mode is
                // limited to an intent-preserving retry of the original
                // instruction; any model rewrite goes through approvals.
                let auto_apply = auto && corrected.trim() == instruction.trim();
...
mod tests {
    ...
    #[tokio::test]
    async fn evaluate_agent_auto_retries_only_the_original_instruction() {
        let host = Arc::new(MockHost::new(tmp_dir()).with_pref(HEALING_AUTO_DECIDE_PREF, "true"));
        host.set_reply(Ok(valid_diagnosis("x", "orig")));
        let engine = HealEngine::new(host);
        let v = engine
            .evaluate(
                "conv-b",
    ...
    #[tokio::test]
    async fn evaluate_agent_queues_model_rewrites_even_when_auto_decide_is_on() {
        let host = Arc::new(MockHost::new(tmp_dir()).with_pref(HEALING_AUTO_DECIDE_PREF, "true"));
        host.set_reply(Ok(valid_diagnosis("x", "rewritten")));
        let engine = HealEngine::new(host);
        let verdict = engine
            .evaluate(
                "conv-rewrite",
        ...
        assert_eq!(
            verdict,
            HealVerdict::QueueFix {
                source_id: "conv-rewrite".into(),
                agent_id: Some("ag".into()),
                diagnosis: "x".into(),
                corrected: "rewritten".into(),
            }
        );
    }
    ...
    async fn report_failure_applies_the_verdict() {
        ...
        engine
            ...
            .await;
        // report_failure -> evaluate -> apply_verdict -> approval queue because
        // the model supplied a rewritten prompt.
        let actions = host.actions();
        assert_eq!(actions.len(), 1);
        assert_eq!(actions[0], "queue_fix:conv-h:Some(\"ag\"):d:fixed prompt");
    }
}
